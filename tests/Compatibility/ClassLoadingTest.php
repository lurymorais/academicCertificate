...
namespace APP\plugins\generic\academicCertificate\tests\compatibility;
...
class ClassLoadingTest {
    ...
    private function testPluginClassReferences() {
        ...
        $file = dirname(__DIR__, 2) . '/AcademicCertificatePlugin.php';
        $content = file_get_contents($file);

        // Check parent class
        if (preg_match('/class\s+AcademicCertificatePlugin\s+extends\s+\\\\PKP\\\\plugins\\\\GenericPlugin/', $content)) {
