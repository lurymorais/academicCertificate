...
All notable changes to **Academic Certificate Manager for OJS** (plugin folder: `academicCertificate`) are documented in this file.
...
### Added — Holistence Publication release

- **Rebrand**: *Academic Certificate Manager for OJS* / *OJS Akademik Belge Yöneticisi* (Holistence Publication)
- **Article acceptance certificates** (`acceptance`) — for authors after editorial accept decision
- **Author publication certificates** (`author`) — for authors after article is published
- **Editorial contribution certificates** (`editor`) — schema and settings groundwork
- **Certificate numbering** — `ACM-{TYPE}-{YEAR}-{SEQ}` format (e.g. `ACM-AUT-2026-000001`)
- **My Certificates / Belgelerim** — role-aware listing with certificate type filter
- **Navigation menu item type** — managers add *Belgelerim* to any menu via Settings → Navigation Menus
- **Per-type PDF templates and backgrounds** — reviewer, acceptance, author, editor
- **Publisher metadata** — Holistence Publication; developers Cumali Yaşar, Mehmet Sahin; contact@holistence.com

### Changed

- **Full plugin rename**: folder `reviewerCertificate` → `academicCertificate`; namespace `APP\plugins\generic\academicCertificate`; locale keys `plugins.generic.academicCertificate.*`; class `AcademicCertificatePlugin`. Database tables unchanged (`reviewer_certificates`, etc.) for upgrade compatibility.
- Plugin `displayName` and `description` updated across all locale files
- Profile page link to Belgelerim retained; automatic menu injection replaced by configurable menu type
- Version `1.8.0.0` in `version.xml`

### Documentation

- `PLUGIN_GALLERY.md` — OJS Plugin Gallery publish guide
- `plugin-gallery-entry.example.xml` — sample `plugins.xml` entry
...
- **Fixed: Ukrainian locale keys showing as `##key##` on reviewer dashboard**
  - **Issue**: When OJS locale is set to Ukrainian (`uk_UA`), the certificate download button on the reviewer dashboard showed raw locale keys (`##plugins.generic.academicCertificate.certificateAvailable##`) instead of translated text. English locale worked fine.
  ...
  - **File Modified**: `classes/AcademicCertificatePluginCore.php`
...
- **Fixed: Memory exhaustion (805MB OOM) on settings page with large reviewer lists**
  ...
  - **Files Modified**: `classes/form/CertificateSettingsForm.php`, `classes/AcademicCertificatePluginCore.php`, `controllers/CertificateHandler.php`
...
- **Fixed: Plugin enable checkbox broken on OJS 3.3 — clicking toggles visually but never fires AJAX**
  ...
  - **Root Cause**: OJS 3.3's base `Plugin::getName()` returns `strtolower(get_class($this))` — the fully qualified class name including namespace backslashes: `app\plugins\generic\reviewercertificate\academiccertificateplugin`. These backslashes get embedded in the checkbox element ID and break the jQuery `pkpHandler` initialization (backslashes are CSS escape characters in selectors).
  - **Solution**: Override `getName()` to return `'academiccertificateplugin'` (simple name without namespace). On OJS 3.4+ this is a no-op since `getName()` already returns the short name.
  - **File Modified**: `classes/AcademicCertificatePluginCore.php`
...
- **Fixed: Handler endpoints return HTTP 500 on OJS 3.3**
  ...
  - **Files Modified**: `classes/AcademicCertificatePluginCore.php`, `controllers/CertificateHandler.php`
...
- **Why `getName()` override works**: The checkbox ID becomes `select-cell-academiccertificateplugin-enabled-...` (no backslashes), the jQuery selector matches, `pkpHandler` initializes, and checkbox clicks trigger the AJAX enable call. `plugin_settings` stores `plugin_name = 'academiccertificateplugin'` consistently across all OJS versions.
- **Why FQN for `HANDLER_CLASS`**: OJS 3.3's `PKPPageRouter::route()` does `get_class_methods(HANDLER_CLASS)` then `new $handlerClass()`. The FQN `'APP\\plugins\\generic\\academicCertificate\\controllers\\CertificateHandler'` resolves correctly because the class file was already loaded via `require_once`.
...
- **Fixed: Plugin won't stay enabled after page refresh on OJS 3.3.0-22**
  ...
  - **Files Modified/Added**:
    - `AcademicCertificatePlugin.php` - Now a simple loader that includes autoloader first, then implementation
    - `compat_autoloader.php` (NEW) - OJS 3.3 compatibility autoloader
    - `classes/AcademicCertificatePluginCore.php` (NEW) - Main plugin implementation moved here
...
- **Two-File Architecture Solution**:
  1. `AcademicCertificatePlugin.php` has NO namespace - it's a simple loader
  2. It first includes `compat_autoloader.php` which registers the autoloader
  3. Then it includes `classes/AcademicCertificatePluginCore.php` which has the actual plugin code with namespace
...
- **Class Map**: The autoloader maps 20+ OJS 3.4+ namespaced classes to their OJS 3.3 global equivalents:
  ...
  - And more...
- **Guard Constant**: Uses `ACADEMIC_CERTIFICATE_COMPAT_AUTOLOADER` to prevent registering the autoloader multiple times
...
- **Fixed: Plugin won't stay enabled after page refresh on OJS 3.3.0-22**
  ...
  - **Files Modified**:
    - `AcademicCertificatePlugin.php` - Added fallbacks for DAORegistry, Application, TemplateManager
...
- **Fixed: Plugin installation fails on OJS 3.3.0-20 through 3.3.0-22**
  ...
  - **File Modified**: `classes/migration/AcademicCertificateInstallMigration.php`
  - **Reported by**: @drugurkocak (Dr. Uğur Koçak)
...
- **Critical: Missing `class_alias()` Calls** - Fixed class inheritance errors in OJS 3.3/3.4
  - **Issue**: `Class "APP\plugins\generic\academicCertificate\mysqli" not found`
...
- **Critical: Unqualified Global Class References** - Fixed PHP global classes not found
  ...
  - **Files Modified**: `AcademicCertificatePlugin.php`, `CertificateHandler.php`, `CertificateGenerator.php`
...
- **Fixed: Incorrect `class_exists()` Check** - Handler loading check used unqualified class name
  ...
  - **Solution**: Use FQN string `'APP\\plugins\\generic\\academicCertificate\\controllers\\CertificateHandler'`
  - **File Modified**: `AcademicCertificatePlugin.php`
...
- **Critical: Missing Use Statements** - Fixed class loading errors in OJS 3.5
  - **Issue**: `Class "APP\plugins\generic\academicCertificate\Application" not found`
  - **Issue**: `Class "APP\plugins\generic\academicCertificate\CertificateSettingsForm" not found`
...
- **Fixed: Tarball Structure** - Plugin files now extract to correct directory
  - **Issue**: Files extracted to root of `plugins/generic/` instead of `plugins/generic/academicCertificate/`
  - **Solution**: Tarball now includes top-level `academicCertificate/` directory wrapper
...
| Old Name | New Name |
|----------|----------|
| `AcademicCertificatePlugin.inc.php` | `AcademicCertificatePlugin.php` |
...
| `classes/migration/AcademicCertificateInstallMigration.inc.php` | `classes/migration/AcademicCertificateInstallMigration.php` |

#### Namespaces Added
- `APP\plugins\generic\academicCertificate` - Main plugin
- `APP\plugins\generic\academicCertificate\controllers` - Request handlers
- `APP\plugins\generic\academicCertificate\classes` - Core classes
- `APP\plugins\generic\academicCertificate\classes\form` - Form classes
- `APP\plugins\generic\academicCertificate\classes\migration` - Database migrations
...
- **Critical: OJS 3.5 URL Parameter Type Error** - Fixed TypeError preventing certificate button display
  ...
  - **Files Modified**: `AcademicCertificatePlugin.inc.php` (lines 529, 658, 660)
  - **Reported by**: @drugurkocak (GitHub Issue #57)
...
- **Critical: OJS 3.5 Handler Registration Error** - Fixed HANDLER_CLASS deprecation error
  ...
  - **Files Modified**: `AcademicCertificatePlugin.inc.php` - `setupHandler()` method
  - **Reported by**: @drugurkocak (GitHub Issue #57)
...
- **Critical: OJS 3.5 Compatibility** - Fixed "Call to undefined function import()" errors preventing plugin installation
  ...
  - Updated AcademicCertificatePlugin.inc.php to use modern PHP namespaces
...
- **Critical: Class Loading Issue** - Fixed "Plugin expected to inherit from AcademicCertificatePlugin, actual type NULL" error
...
- **Critical: AJAX Settings Form Error** - Fixed "Failed Ajax request or invalid JSON returned" error when clicking Settings button
  ...
  - Files: `AcademicCertificatePlugin.inc.php`, `CertificateSettingsForm.inc.php`
- **Critical: Database Migration Failures in OJS 3.3** - Tables not being created during automatic installation
  ...
  - File: `classes/migration/AcademicCertificateInstallMigration.inc.php`
...
- **No database changes** - Safe upgrade with no migration required
- **No configuration changes** - All existing settings preserved
- **Tarball fix**: Plugin now extracts to correct `academicCertificate/` directory
- **Recommended**: Clear OJS cache after upgrade (`php tools/upgrade.php check`)
...
## Links
- **Repository**: https://github.com/ssemerikov/academicCertificate
- **Issues**: https://github.com/ssemerikov/academicCertificate/issues
