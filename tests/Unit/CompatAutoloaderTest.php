...
namespace APP\plugins\generic\academicCertificate\tests\Unit;
...
class CompatAutoloaderTest extends TestCase {
    ...
    protected function setUp(): void {
        ...
        $this->mainPluginPath = dirname(__DIR__, 2) . '/AcademicCertificatePlugin.php';
        $this->corePluginPath = dirname(__DIR__, 2) . '/classes/AcademicCertificatePluginCore.php';
    }
    ...
    public function testMainPluginIsLoader(): void {
        ...
        $this->assertMatchesRegularExpression(
            '/require_once.*AcademicCertificatePluginCore\.php/',
            $content,
    ...
    public function testCorePluginHasNamespace(): void {
        ...
        $this->assertMatchesRegularExpression(
            '/^namespace\s+APP\\\\plugins\\\\generic\\\\academicCertificate;/m',
            $content,
    ...
    public function testAutoloaderHasGuardConstant(): void {
        ...
        $this->assertStringContainsString(
            'ACADEMIC_CERTIFICATE_COMPAT_AUTOLOADER',
            $content,
            'Autoloader should use a guard constant to prevent duplicate registration'
        );

        $this->assertMatchesRegularExpression(
            '/if\s*\(\s*!defined\s*\(\s*[\'"]ACADEMIC_CERTIFICATE_COMPAT_AUTOLOADER[\'"]\s*\)\s*\)/',
            $content,
    ...
    public function testTwoFileArchitectureOrder(): void {
        ...
        $corePos = strpos($content, 'AcademicCertificatePluginCore.php');
    ...
    public function testMainPluginCreatesGlobalAlias(): void {
        ...
        $this->assertMatchesRegularExpression(
            '/class_alias\s*\(\s*[\'"]APP\\\\\\\\plugins\\\\\\\\generic\\\\\\\\academicCertificate\\\\\\\\AcademicCertificatePlugin[\'"]/',
            $content,
    ...
    public function testCorePluginExtendsGenericPlugin(): void {
        ...
        $this->assertMatchesRegularExpression(
            '/class\s+AcademicCertificatePlugin\s+extends\s+GenericPlugin/',
            $content,
