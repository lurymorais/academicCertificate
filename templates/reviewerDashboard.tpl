{**
 * plugins/generic/academicCertificate/templates/reviewerDashboard.tpl
 *
 * Copyright (c) 2024
...
{if $showCertificateButton && $certificateUrl}
	<div class="reviewer-certificate-section" style="margin: 20px 0; padding: 15px; background: #f0f8ff; border: 1px solid #b0d4f1; border-radius: 5px;">
		<h3 style="margin-top: 0; color: #0066cc;">
			...
			{translate key="plugins.generic.academicCertificate.certificateAvailable"}
		</h3>
		<p style="margin: 10px 0;">
			{translate key="plugins.generic.academicCertificate.certificateAvailableDescription"}
		</p>
		...
		{if !$certificateExists}
			<p class="description" style="margin-top: 10px; font-size: 0.9em; color: #666;">
				{translate key="plugins.generic.academicCertificate.certificateWillBeGenerated"}
			</p>
		{/if}
		<p style="margin-top: 10px;">
			<a href="{url page="certificate" op="myCertificates"}" style="color: #0066cc; text-decoration: underline;">
				{translate key="plugins.generic.academicCertificate.myCertificates.viewAll"}
			</a>
		</p>
	</div>
{/if}
