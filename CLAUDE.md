...
Root namespace: `APP\plugins\generic\academicCertificate` → `./` (defined in `composer.json`). All new classes must use this namespace prefix. Test namespace: `APP\plugins\generic\academicCertificate\Tests` → `tests/`.
...
```
AcademicCertificatePlugin.php  (entry point — thin wrapper)
  → compat_autoloader.php      (registers spl_autoload with prepend=true)
  → classes/AcademicCertificatePluginCore.php  (actual implementation)
...
- `AcademicCertificatePlugin.php` — Entry point (thin wrapper, namespace `APP\plugins\generic\academicCertificate`)
- `compat_autoloader.php` — Namespace compatibility layer (OJS 3.3 ↔ 3.4+)
- `classes/AcademicCertificatePluginCore.php` — Actual implementation. Hooks: `LoadHandler`, `TemplateManager::display`, `reviewassignmentdao::_updateobject`
...
Two-path strategy in `classes/migration/AcademicCertificateInstallMigration.php`:
...
```bash
./release.sh 1.6.0
# Produces: academicCertificate-1.6.0-3_3.tar.gz, -3_4.tar.gz, -3_5.tar.gz
```

**Release packages:**
- `academicCertificate-{VERSION}-3_3.tar.gz` — Includes `compat_autoloader.php` + TCPDF
- `academicCertificate-{VERSION}-3_4.tar.gz` — TCPDF only (no compat_autoloader)
- `academicCertificate-{VERSION}-3_5.tar.gz` — TCPDF only (no compat_autoloader)
